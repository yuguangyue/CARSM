...
################################ build agent ##################################
network = 'Linear'
model = ARSM_AGENT_mujoco(nA, network, tau, Grad_Clip_Norm, K, C)
logger.configure()
###############################################################################
...
while (timestep < total_timesteps):
    optimizer_actor = tf.train.AdamOptimizer(scheduler(lr_actor, total_timesteps, timestep, 1))
    optimizer_q = tf.train.AdamOptimizer(scheduler(lr_q, total_timesteps, timestep, 1))    
    ...
    runner_mujoco(env, nsteps, model, gamma, initial_state, nA, env_type, replay_buffer,
                  K, C, action_high, action_low, random_choose, one_hot = False)
    sample_time += 1
    timestep += len(dones)
...
###############################################################################
        ...
        for _ in range(n_iter_policy):
            grad_policy = model.update_policy(states, optimizer_actor, f_delta, entropy_par, K, C)
        ...
        tmp = tf.concat(
              [tf.reshape(g, [-1]) for g in grad_policy if g is not None], axis=0)
        pg_norm = tf.norm(tmp)
        tmp = tf.concat(
